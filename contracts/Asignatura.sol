...
contract Asignatura {
    ...
    /**
     * Devuelve la nota final que ha sacado el alumno en la asignatura.
     * 
     * @param alumno Direccion del alumno.
     * 
     * @return nota La nota qur ha sacado el alumno, multiplicada por 1000.
     */
    
    function notaFinal(address alumno) public view returns(uint nota) {
        nota = 0;
        uint notaEv;

        for (uint i=0; i<evaluaciones.length;i++){
            
            if(calificaciones[alumno][i].tipo != TipoNota.NP) {
                notaEv = calificaciones[alumno][i].calificacion * evaluaciones[i].puntos;
            } else {
                notaEv = 0;
            }
            nota += notaEv;
