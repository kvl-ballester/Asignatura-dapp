...
const CalificacionRow = (props) => {
    ...
    for (let ei = 0; ei < evaluacionesLength; ei++) {
        ...
        cells.push(
            <ContractData drizzle={drizzle} drizzleState={drizzleState}
                          contract={"Asignatura"} method={"calificaciones"} methodArgs={[alumnoAddr, ei]}
                          render={nota =>
    ...
    return (
        <tr key={"d" + alumnoIndex}>
            ...
            <td><ContractData drizzle={drizzle} drizzleState={drizzleState}
                              contract={"Asignatura"} method={"datosAlumno"} methodArgs={[alumnoAddr]}
                              render={datos => <>
                                    {datos.nombre}
                                </>
                              }
